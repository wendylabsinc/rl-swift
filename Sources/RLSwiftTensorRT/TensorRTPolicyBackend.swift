...
/// Errors thrown by the RLSwift TensorRT backend when model IO cannot be mapped
/// to RLSwift policy inputs or robot actions.
...
/// Configuration for running a TensorRT engine as a RLSwift policy backend.
...
/// TensorRT inference output converted into a RLSwift robot action.
...
public struct TensorRTBackendSupport: Equatable, Sendable {
    ...
    public static let current = TensorRTBackendSupport(
        ...
        explanation: "TensorRT backend is Linux-only; use RLSwift or RLSwiftMLX on Apple devices and RLSwiftTensorRT on NVIDIA Linux."
    )
#endif
}

#if os(Linux) && SWIFTRL_ENABLE_TENSORRT
/// A TensorRT-backed RLSwift policy adapter for low-latency robot inference.
///
