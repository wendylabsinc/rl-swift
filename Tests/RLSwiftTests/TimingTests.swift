...
@Suite struct TimingTests {
    @Test func validatesTimingDurations() {
        #expect(throws: RLSwiftError.invalidDuration(name: "deltaTime", value: -0.1)) {
            _ = try ControlTiming(stepIndex: 0, deltaTime: -0.1)
        }
        #expect(throws: RLSwiftError.invalidDuration(name: "sensorAge", value: -0.1)) {
            _ = try ControlTiming(stepIndex: 0, deltaTime: 0.02, sensorAge: -0.1)
        }
        #expect(throws: RLSwiftError.invalidDuration(name: "actionLatency", value: -0.1)) {
    ...
    @Test func computesClosedLoopLatencyAndDeadlines() throws {
        ...
        #expect(throws: RLSwiftError.invalidDuration(name: "maximumLatency", value: -1)) {
